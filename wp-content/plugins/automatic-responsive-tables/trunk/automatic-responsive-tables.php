	...
	class responsive_tables {
		...
		protected $main_settings = array(
			...
			'exclude_classes' => array(
				'title' => 'Exclude Classes or IDs',
				'description' => 'Add the Classes or IDs of the tables that you would like to EXCLUDE from the convertion process. <br /> ( separated by commas )',
				'type' => 'text',
				'placeholder' => '.class, #id'
			),
			'breakpoint' => array(
		...
		public function settings_page() {
			...
			if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == true ) {
				...
				if ( ! $breakpoint || is_nan( $breakpoint ) ) {
					$breakpoint = 800;
				}
				...
				if( isset( $main_options['activate_all'] ) ) {
					...
					if( isset( $main_options['exclude_classes'] ) && $main_options['exclude_classes'] != '' ) {
						$jquery_selector .= ':not(' . $main_options['exclude_classes'] . ')';
					}
					$frontend_output .= $jquery_selector . ' { display: none !important; }' . PHP_EOL;
				...
				$js_path = __DIR__ . '/js/script-template.js';
				$js_file = file( $js_path );
				if( strpos($js_file[0],'var queryString') !== false ) {
					$js_file[0] = 'var queryString = "' . $jquery_selector . '";' . PHP_EOL;
				}
				if( strpos($js_file[1],'var breakpoint') !== false ) {
					$js_file[1] = 'var breakpoint = ' . $breakpoint . ';' . PHP_EOL;
				}
				file_put_contents( __DIR__ . '/js/script.js', $js_file );
			}

			//---- Check to make sure plugin has it's needed permissions
			$js_path = __DIR__ . '/js/script.js';
			if ( ! is_writable( $js_path ) ) {
				echo <<<HTML
				    <div class="notice notice-warning is-dismissible">
				        <p>The plugin needs permission to `file_put_contents` in order to work properly.</p>
				    </div>
HTML;
			}

			// make sure there are some type of tings for the plugin to process
			if (
				( ! isset( $main_options['classes'] ) || $main_options['classes'] == '' )
				&& ! isset( $main_options['activate_all'] )
			) {
				echo <<<HTML
				    <div class="notice notice-warning is-dismissible">
				        <p>You need to either "Activate for all Tables" or supple specific "Classes & IDs" for this plugin to begin working.</p>
				    </div>
HTML;
			}
	...
	function add_rt_admin_script () {
		...
		if( $screen->id == 'settings_page_responsive-tables' ) {
	        ...
	        wp_enqueue_script( 'rt_color_picker', plugins_url( 'js/script-admin.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), false, true ); 
		}
	}
	...
	function add_rt_frontend_script () {
		wp_enqueue_script( 'rt_frontend_script', plugins_url( 'js/script.js', __FILE__ ), [ 'jquery' ] );
