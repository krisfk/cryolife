jQuery(document).ready(function($) {
    ...
    $('.wp-picker-container').on('mousemove mouseup keyup mousedown click', updateColorPickerStyle );
    $('.wp-picker-container').each( updateColorPickerStyle );
    ...
    $('#header_font').on('change mouseup keyup', updateHeaderFontStyle );
    function updateHeaderFontStyle() {
    	var fontSize = $('#header_font').val();
    	if( $('#header_font').val() == '') {
    		fontSize = 16;
    	}
    ...
    $('#value_font').on('change mouseup keyup', updateValueFontStyle );
    function updateValueFontStyle() {
    	var fontSize = $('#value_font').val();
    	if( fontSize == '' ) {
    		fontSize = 16;
    	}
    	$('.art-table .art-tbody .art-tr .art-td .art-td-last').css('font-size', fontSize+'px');
    }
    updateValueFontStyle();


    $('#header_value_divider').click( updateHeaderDividerStyle )
	function updateHeaderDividerStyle() {
		if( $('#header_value_divider').is(':checked') ) {
