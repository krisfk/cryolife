...
function uncode_page_options()
{
	...
	*/
	...
	if ($post_type === 'post' || $post_type === 'page' || $post_type === 'portfolio' ) {
		$body_fields[] = run_array_mb($specific_media_display, '_uncode_specific_media:not(no)');
	}
	$body_fields[] = run_array_mb($specific_featured_media, '_uncode_specific_media:not(on)');
	$body_fields[] = run_array_mb($specific_share);
	...
	$footer_fields = array(
		...
		run_array_mb($specific_footer_width,'_uncode_specific_copy_hide:not(on)') ,
	);
