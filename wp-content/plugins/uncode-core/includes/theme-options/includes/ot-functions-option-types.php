	  ...
	  echo '<div class="format-setting-inner">';
		...
		if ( is_array( $my_posts ) && ! empty( $my_posts ) ) {
			...
			foreach( $my_posts as $my_post ) {
				...
				if (($field_value == $my_post->ID) && $post_link !== '') {
					$post_active = $post_link;
					if ( function_exists( 'vc_frontend_editor' ) ) {
						$post_active_frontend = esc_url( vc_frontend_editor()->getInlineUrl( '', $my_post->ID) );
					}
				}
				$post_link = ($post_link !== '') ? ' data-link="'. $post_link . '"' : '';
				if ( function_exists( 'vc_frontend_editor' ) ) {
					$post_link .= ' data-frontend-link="' . esc_url( vc_frontend_editor()->getInlineUrl( '', $my_post->ID) ) . '"';
				}
	  ...
	  echo '<div class="format-setting-inner">';
		...
		foreach ( (array) $field_choices as $choice ) {
		  if ( isset( $choice['value'] ) && isset( $choice['label'] ) ) {
			...
			if ( function_exists( 'vc_frontend_editor' ) && $vc_editable ) {
