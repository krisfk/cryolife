	...
	class Uncode_DOT_IRecommendThis {
		...
		function dot_enqueue_scripts()
		{
			if ( apply_filters('uncode_dot_irecommendthis', false) ) {
				...
				wp_localize_script( 'uncode-irecommendthisd', 'uncode_irecommendthis', array(
					'i18n'    => esc_html__( 'You already recommended this', 'uncode-core' ),
		...
		function dot_recommend_this($post_id, $action = 'get')
		{
			...
			switch ($action)
			{
				case 'get':
					...
					if ($recommended == 0)
					{
						$output = '<span class="extras-wrap"><i class="fa fa-heart3"></i><span><span class="likes-counter">0</span> ' . esc_html__('Like', 'uncode-core') . '</span></span>';
						return $output;
					}
					else
					{
						$output = '<span class="extras-wrap"><i class="fa fa-heart3"></i><span><span class="likes-counter">' . esc_html( $recommended ) . '</span> ' . esc_html__('Likes', 'uncode-core') . '</span></span>';
						return $output;
					}
		...
		function dot_recommend($id = null, $wrap = true)
		{
			...
			if (!isset($_COOKIE['dot_irecommendthis_' . $post_ID]) && $voteStatusByIp == 0) {
				...
				$title = esc_html__('Recommend this', 'uncode-core');

			} else {

				$class = 'uncode-dot-irecommendthis active';
				$title = esc_html__('You already recommended this', 'uncode-core');
			}
	...
	class Uncode_Most_recommended_posts extends WP_Widget
	{
		...
		function __construct()
		{
			parent::__construct('most-recommended-posts', // Base ID
				esc_html__('Most recommended posts', 'uncode-core') , // Name
				array('description' => esc_html__('Your site’s most liked posts.', 'uncode-core') ,) // Args
			);
		}
		...
		{
			$title = !empty($instance['title']) ? $instance['title'] : esc_html__('Most recommended posts', 'uncode-core');
			$number = !empty($instance['number']) ? $instance['number'] : 5;
			$show_count = !empty($instance['show_count']) ? true : false;
			?>
	    <p>
	        <label for="<?php echo  esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'uncode-core'); ?></label>
	        <input class="widefat" id="<?php echo  esc_attr($this->get_field_id('title')); ?>" name="<?php echo  esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>">
	    </p>
	    <p>
	        <label for="<?php echo  esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Number of posts to show:', 'uncode-core'); ?></label><br />
	    ...
	    <p>
	        <label for="<?php echo  esc_attr($this->get_field_id('show_count')); ?>"><?php esc_html_e('Show post count', 'uncode-core'); ?></label>
...
if ( ! function_exists( 'uncode_dot_columns_head' ) ) {
	function uncode_dot_columns_head($defaults) {
		...
		$defaults['likes'] = esc_html__('Likes', 'uncode-core');
		return $defaults;
	}
}
...
if ( ! function_exists( 'uncode_dot_column_content' ) ) {
	function uncode_dot_column_content($column_name, $post_ID) {
		...
		if ($column_name == 'likes') {
			echo esc_html( get_post_meta($post_ID, '_recommended', true) . ' ' . __('like', 'uncode-core') );
		}
	}
}
