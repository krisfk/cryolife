...
function uncode_core_system_status_print_memory() {
	...
	<tr>
		<td data-export-label="Server Memory Limit"><?php esc_html_e( 'Server Memory Limit', 'uncode-core' ); ?>
		<?php echo '<span class="toggle-description"></span><small class="description">' . esc_attr__( 'This is actually the real memory available for your installation despite the WP memory limit.', 'uncode-core' ) . '</small>'; ?></td>
		<td class="real-memory">
			...
			<mark class="saved-memory <?php echo $memory_is_ok ? 'yes' : 'error'; ?>">
				...
				<?php else : ?>
					<?php echo sprintf( esc_html__( 'You only have %s available and it\'s not enough to run the system. If you have already increased the memory limit please check with your hosting provider for increase it (at least 96MB is required)','uncode-core' ), $memory_formatted_text ); ?>
				<?php endif; ?>
			</mark>
			<span class="calculating" style="display: none;"><?php esc_html_e( 'Calculating…', 'uncode-core' ); ?></span>
			<mark class="yes" style="display: none;">%d% MB</mark>
			<mark class="error" style="display: none;"><?php esc_html_e( 'You only have %d% MB available and it\'s not enough to run the system. If you have already increased the memory limit please check with your hosting provider for increase it (at least 96MB is required)','uncode-core' ); ?></mark>
