 ...
 */
function voodioo_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->voodioo_comment_list_args = $args;
	$comment = $_comment;
 ...
 */
function voodioo_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->voodioo_comment_list_args['avatar_size'] ) ) {
		$size = $comment->voodioo_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'voodioo_comment_author_avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function voodioo_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'voodioo_get_comment_author_link', sprintf( '<b class="fn">%s</b>', get_comment_author_link() ), $args );
}
 ...
 */
function voodioo_get_comment_date( $args = array() ) {
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'voodioo_get_comment_date', sprintf( '<span class="comment-date">%4$s <a href="%3$s" class="comment-date__link"><time datetime="%1$s" class="comment-date__time">%2$s</time></a></span>', get_comment_time( 'c' ), get_comment_date( $format ), get_comment_link(), esc_html__( 'on', 'voodioo' ) ), $args );
}
 ...
 */
function voodioo_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'voodioo_get_comment_text', $comment_text, $args );
}
 ...
 */
function voodioo_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'voodioo_get_comment_reply_link', $reply, $args );
}
 ...
 */
function voodioo_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', 'voodioo' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'voodioo_get_comment_link_edit', $link, $comment->comment_ID, $args );
}
