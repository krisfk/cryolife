 ...
 * @return array
 */
function voodioo_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! voodioo_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'voodioo_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'voodioo_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'voodioo' ) . $network['name'],
	...
	$share_before_html = apply_filters( 'voodioo_share_toggle_html', '<div class="share-btns__before"><i class="nc-icon-outline ui-2_share-bold"></i></div>' );

	printf(
		'<div class="share-btns__list %1$s">%2$s<div class="share-btns__items">%3$s</div></div>',
 ...
 * @return string
 */
function voodioo_get_single_share_buttons() {
	ob_start();
