...
class Voodioo_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'voodioo' );
		$this->widget_id          = 'voodioo_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'voodioo' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'voodioo' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'voodioo' ),
				'label' => esc_html__( 'Subscribe Title', 'voodioo' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'voodioo' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'voodioo' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'voodioo' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'voodioo' ),
				'label' => esc_html__( 'Subscribe submit label', 'voodioo' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'voodioo' ),
				'label' => esc_html__( 'Subscribe success', 'voodioo' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'voodioo' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'voodioo' ),
				'label' => esc_html__( 'Follow Title', 'voodioo' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'voodioo' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'voodioo' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'voodioo' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'voodioo' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'voodioo' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'voodioo' ),
				'value'   => 'center',
				'options' => array(
			...
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'voodioo' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'voodioo' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'voodioo' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'voodioo' ),
				'value'   => 'inherit',
				'options' => array(
			...
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'voodioo' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_voodioo_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_voodioo_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'voodioo' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'voodioo' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'voodioo' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'voodioo' ),
		);

		$this->nonce = wp_create_nonce( 'voodioo-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'voodioo' );
		}
	...
	public function get_social_nav() {
		return voodioo_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = voodioo_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'voodioo-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'voodioo_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function voodioo_register_subscribe_follow_widgets() {
	register_widget( 'Voodioo_Subscribe_Follow_Widget' );
}
