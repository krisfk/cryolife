 ...
 * @package Voodioo
 */
if ( ! class_exists( 'Voodioo_About_Author_Widget' ) ) {

	/**
	 * Class Voodioo_About_Author_Widget.
	 */
	class Voodioo_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'voodioo widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'voodioo' );
			$this->widget_id          = 'voodioo_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'voodioo' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'voodioo' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'voodioo' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'voodioo' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'voodioo' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'voodioo' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'voodioo' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'voodioo' ),
					'label' => esc_html__( 'Link label', 'voodioo' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'voodioo' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'voodioo-author-avatar' );
...
add_action( 'widgets_init', 'voodioo_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function voodioo_register_about_author_widgets() {
	register_widget( 'Voodioo_About_Author_Widget' );
}
