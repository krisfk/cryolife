...
if ( ! class_exists( 'Voodioo_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Voodioo_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'voodioo' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'voodioo' );
			$this->widget_id          = 'voodioo-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'voodioo' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'voodioo' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'voodioo' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'voodioo' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'voodioo' ),
						),
					),
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'voodioo' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'voodioo_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'voodioo_taxonomy_tiles_widget_before',
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s post', '%s posts', 'voodioo' ),
				...
				echo apply_filters( 'voodioo_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'voodioo_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function voodioo_register_taxonomy_tiles_widget() {
		register_widget( 'Voodioo_Taxonomy_Tiles_Widget' );
	}
}
