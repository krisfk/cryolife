...
if ( ! class_exists( 'Voodioo_Custom_Posts_Widget' ) ) {

	/**
	 * Class Voodioo_Custom_Posts_Widget.
	 */
	class Voodioo_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'voodioo' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'voodioo' );
			$this->widget_id          = apply_filters( 'voodioo_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'voodioo_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = voodioo_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'voodioo' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'voodioo' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'voodioo' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'voodioo' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'voodioo' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'voodioo' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'voodioo' ),
						'category'      => esc_html__( 'Category', 'voodioo' ),
						'post_tag'      => esc_html__( 'Tag', 'voodioo' ),
						'more_button'   => esc_html__( 'More Button', 'voodioo' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'voodioo' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', voodioo_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					$image = $this->utility->media->get_image( array(
						'size'              => 'voodioo-thumb-s-2',
						'mobile_size'       => 'voodioo-thumb-s-2',
	...
	add_action( 'widgets_init', 'voodioo_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function voodioo_register_custom_posts_widget() {
		register_widget( 'Voodioo_Custom_Posts_Widget' );
	}
}
