 ...
 * @package Voodioo
 */
if ( ! class_exists( 'Voodioo_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Voodioo_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'voodioo' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'voodioo' );
			...
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => 'Image Grid',
					'label' => esc_html__( 'Widget title', 'voodioo' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category_name',
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'voodioo' ),
				...
				'tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'voodioo' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'voodioo' ),
						'title'         => esc_html__( 'Post Title', 'voodioo' ),
						'comment_count' => esc_html__( 'Comment Count', 'voodioo' ),
					),
					'label'   => esc_html__( 'Post sorted', 'voodioo' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'voodioo' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'voodioo' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'voodioo' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'voodioo_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'voodioo_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'voodioo_image_grid_widget_grid_class', array(
				...
				if( '0' !== $items_padding ){
					...
					voodioo_theme()->dynamic_css->add_style(
				...
				echo apply_filters(
					'voodioo_image_grid_widget_before',
				...
				foreach ( $posts as $post ) {
					...
					$terms = $this->utility->meta_data->get_terms( array(
						'type'      => $taxonomy_type,
						'before'    => '<div class="widget-image-grid__terms post__terms">',
						'after'     => '</div>',
					) );
				...
				echo apply_filters( 'voodioo_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'voodioo_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function voodioo_register_image_grid_widget() {
		register_widget( 'Voodioo_Image_Grid_Widget' );
	}
}
