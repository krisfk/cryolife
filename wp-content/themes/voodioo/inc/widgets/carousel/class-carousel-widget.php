 ...
 * @package Voodioo
 */
if ( ! class_exists( 'Voodioo_Carousel_Widget' ) ) {

	/**
	 * Class Voodioo_Carousel_Widget.
	 */
	class Voodioo_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'voodioo' );
			$this->widget_id          = 'voodioo_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'voodioo' );
			...
			$this->settings           = array(
				...
				'more_button_text' => array(
					'type'   => 'text',
					'value'  => esc_html__( 'Learn more', 'voodioo' ),
				...
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 4,
					'label'     => esc_html__( 'Number of slides per view', 'voodioo' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'voodioo' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'voodioo' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 0,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'voodioo' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'voodioo' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'voodioo' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'voodioo' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			// Widgets area check.
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', voodioo_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="voodioo-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'voodioo' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'voodioo_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'voodioo' ) . '</h5>';
			}
	...
	add_action( 'widgets_init', 'voodioo_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function voodioo_register_carousel_widget() {
		register_widget( 'Voodioo_Carousel_Widget' );
	}
}
