...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					if ( !$skip_td ) {
						...
						echo '<td class="' . esc_attr( $td_class ) . ' woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . '" colspan="' . esc_attr( $colspan ) . '">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button alt btn btn-default <?php echo uncode_btn_style(); ?>"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
		<?php // END UNCODE EDIT ?>
